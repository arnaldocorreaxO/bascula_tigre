...
class ReportForm(forms.Form):
    ...
    (item.id, item.denominacion) for item in Asociacion.objects.filter(activo__exact=True).order_by('denominacion')])
    cliente = forms.ChoiceField(choices=[
    (item.id, item.denominacion) for item in Cliente.objects.filter(activo__exact=True).order_by('denominacion')])
    producto = forms.ChoiceField(choices=[
    (item.id, item.denominacion) for item in Producto.objects.filter(activo__exact=True).order_by('denominacion')])
    chofer = forms.ChoiceField(choices=[
    (item.id, item.nombre + ' ' + item.apellido) for item in Chofer.objects.filter(activo__exact=True).order_by('apellido','nombre')])
